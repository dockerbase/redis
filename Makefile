...
run:
	docker run -p 6379:6379 --name $(subst /,-,$(NAME)) --restart=always -t --cidfile cidfile -d $(NAME):$(VERSION) /sbin/runit
...
version:
	...
	docker run -it --rm $(NAME):$(VERSION) sh -c " redis-server --version ; redis-cli --version " | tee -a COMPONENTS
	dos2unix COMPONENTS
